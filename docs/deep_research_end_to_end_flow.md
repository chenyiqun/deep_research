...
`search_live`、`search_lite` 和 `search_plus`。当前部署默认使用可用的 `search_prime`（Google）。
在 URL fetch 策略为 `auto` 时，支持搜狗的 endpoint 可把其 reader-ready content 直接交给 Reader，不再抓取网页；其余引擎
...
若达到唤醒条件，Main 读取全局摘要和本批新事件，输出增量 DAG Patch；否则 Scheduler 继续推进已有 DAG。
...
Writer 先依据 Claim Ledger 生成 evidence-backed outline，再写正文。每个可核验事实引用 Claim/Evidence ID；不允许从模型记忆补充未入账事实。
...
- pass：进入 Release。
- fail：生成精确 repair items，回到 Main 建立定向研究或删除/降级 Claim。
...
8. Audit 失败必须回到 repair DAG，不能只让 Writer“凭感觉润色”。
